...
apiRouter.post('/auth/admin-login', (req: Request, res: Response) => {
  ...
  if (!user || (user.role !== 'admin' && user.clearanceLevel < 5)) {
    ...
    return res.status(401).json({ error: 'ACCESS DENIED: Administrative credentials required' });
  }

  const calculatedHash = hashPassword(password, user.salt);
  if (calculatedHash !== user.passwordHash) {
    aegisDb.logActivity(username, 'ADMIN_LOGIN_FAILED', username, 'FAILED', 'Incorrect admin password');
    return res.status(401).json({ error: 'ACCESS DENIED: Incorrect credentials' });
  }
...
// (SERVER-SIDE DETERMINED AND VALIDATED)
// ==========================================
...
apiRouter.post('/portal/orders', (req: Request, res: Response) => {
  ...
  if (!moduleId || !planId) {
    return res.status(400).json({ error: 'Missing moduleId or planId' });
  }
...
apiRouter.get('/admin/users', requireAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  const filtered = enrichedUsers.filter(
    (u) =>
...
apiRouter.post('/admin/users', requireAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  const targetId = (authorisedId || username || '').trim().toUpperCase();
  const targetPassKey = (passKey || password || '').trim();

  if (!targetId || !targetPassKey) {
    return res.status(400).json({ error: 'Authorised Account ID and Pass Key are required' });
  }

  const existing = aegisDb.getUserByUsername(targetId);
  if (existing) {
    return res.status(409).json({ error: `Account with ID "${targetId}" already exists` });
  }
  ...
  const newUser = aegisDb.createUser({
    ...
    role: role || 'user',
    clearanceLevel: clearanceLevel ? parseInt(clearanceLevel, 10) : 3,
    email: email || `${targetId.toLowerCase()}@aegis-defense.internal`,
    nodeRegion: nodeRegion || 'Asia-SE',
    accountStatus: accountStatus || 'active',
  });

  // If custom pricing specified during creation
  if (customPricing && (customPricing.plan15Price !== undefined || customPricing.plan30Price !== undefined)) {
  ...
  // If initial module / runtime license specified during creation
  ...
  if (initialModuleId) {
    ...
    initialLicense = aegisDb.provisionLicense(
      ...
      initialDurationDays !== undefined ? Number(initialDurationDays) : 30,
      req.userSession!.username
    );
  }
  ...
  aegisDb.logActivity(
    ...
    newUser.username, 
    'SUCCESS', 
    `Authorised Account ${newUser.username} generated with Level ${newUser.clearanceLevel} clearance.`
  );
...
apiRouter.get('/admin/pricing', requireAdmin, (_req: AuthenticatedRequest, res: Response) => {
  ...
  return res.json({ customPricings, globalPlans });
});
...
apiRouter.put('/admin/pricing/:userId', requireAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  if (
    ...
    return res.status(400).json({ error: 'All 4 plan prices must be specified' });
  }
  ...
  return res.json({ success: true, customPricing: updated });
});
...
apiRouter.delete('/admin/pricing/:userId', requireAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  return res.json({ success: true, reset: deleted });
});
...
apiRouter.post('/admin/modules', requireAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  return res.status(201).json({ success: true, module: mod });
});
...
apiRouter.get('/admin/settings', requireAdmin, (_req: AuthenticatedRequest, res: Response) => {
  return res.json({ settings: aegisDb.getSettings() });
});
